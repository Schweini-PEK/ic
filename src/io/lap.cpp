...
namespace ichol::io
{
    ...
    {
        ichol::matrix::CsrMatrix<T> laplacian;
        laplacian.num_rows = n * n * n;
        laplacian.num_cols = n * n * n;
        // Store only lower triangle + diagonal to match solver expectations.
        laplacian.nnz = 4 * n * n * n - 3 * n * n;
        laplacian.row_ptr.resize(laplacian.num_rows + 1);
        laplacian.col_ind.resize(laplacian.nnz);
        laplacian.values.resize(laplacian.nnz);
        ...
        for (int k = 0; k < n; ++k)
        {
            for (int i = 0; i < n; ++i)
            {
                for (int j = 0; j < n; ++j)
                {
                    ...
                    // Back neighbor (lower triangle)
                    if (k > 0)
                    {
                        laplacian.col_ind[nnz_index] = row - n * n;
                        laplacian.values[nnz_index] = -1;
                        nnz_index++;
                    }
    ...
    template ichol::matrix::CsrMatrix<double> gen_2dlap_csr(int n);
    template ichol::matrix::CsrMatrix<float> gen_2dlap_csr(int n);
    template ichol::matrix::CsrMatrix<half_float::half> gen_2dlap_csr(int n);
    template ichol::matrix::CsrMatrix<double> gen_3dlap_csr(int n);
    template ichol::matrix::CsrMatrix<float> gen_3dlap_csr(int n);
    template ichol::matrix::CsrMatrix<half_float::half> gen_3dlap_csr(int n);
} // namespace ichol::io
