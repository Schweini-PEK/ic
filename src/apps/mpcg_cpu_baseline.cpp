...
struct GpuSubdomainContextDeleter
{
...
struct CpuSubdomainContextDeleter
{
    void operator()(ichol::precond::SubdomainIncompleteCholeskyMklContext *ctx) const
    {
        if (ctx != nullptr)
...
using GpuSubdomainContextPtr = std::unique_ptr<ichol::precond::SubdomainSpSVContext, GpuSubdomainContextDeleter>;
using CpuSubdomainContextPtr = std::unique_ptr<ichol::precond::SubdomainIncompleteCholeskyMklContext, CpuSubdomainContextDeleter>;

struct GpuSubdomainBundle
{
    std::vector<ichol::precond::SubdomainRegion> regions;
    std::vector<GpuSubdomainContextPtr> contexts;
    std::vector<ichol::precond::PrecondApply> preconds;
    double build_secs = 0.0;
};

struct CpuSubdomainBundle
{
    std::vector<ichol::precond::SubdomainRegion> regions;
    std::vector<CpuSubdomainContextPtr> contexts;
    std::vector<ichol::precond::PrecondApply> preconds;
    double build_secs = 0.0;
};
...
ichol::precond::SubdomainPreconditionerKind parse_precond_kind(const std::string &raw)
{
    ...
    if (v == "exact" || v == "exactcholesky" || v == "fsai" || v == "spai")
        throw std::runtime_error("mpcg_cpu_baseline currently supports only --precond ic");
...
void print_usage(const char *argv0)
{
    std::cout
        ...
        << "  --precond ic                  shared GPU/CPU DD preconditioner family\n"
...
GpuSubdomainBundle build_gpu_subdomain_bundle(const ichol::matrix::CsrMatrix<double> &A,
...
CpuSubdomainBundle build_cpu_subdomain_bundle(const ichol::matrix::CsrMatrix<double> &A,
                                              const AppOptions &opts)
{
    const ichol::precond::GridShape global_shape{opts.n, opts.n, opts.n};
    const ichol::precond::SubdomainSize subdomain_size{opts.subdomain.x, opts.subdomain.y, opts.subdomain.z};

    CpuSubdomainBundle bundle;
    bundle.regions = ichol::precond::partition_subdomains(global_shape, subdomain_size);
    if (bundle.regions.empty())
        throw std::runtime_error("partition_subdomains returned no regions");
...
SolverRun run_gpu_mpcg(const ichol::matrix::CsrMatrix<double> &A,
                       const std::vector<double> &b,
                       const AppOptions &opts,
                       const GpuSubdomainBundle &bundle)
{
    SolverRun run;
...
SolverRun run_cpu_mpcg(const ichol::matrix::CsrMatrix<double> &A,
                       const std::vector<double> &b,
                       const AppOptions &opts,
                       const CpuSubdomainBundle &bundle)
{
    SolverRun run;
    run.precond_secs = bundle.build_secs;
    ...
    std::vector<double> x(A.num_rows, 0.0);
    const auto t0 = std::chrono::high_resolution_clock::now();
    run.result = ichol::solver::mpcg_cpu_baseline<double>(
        A.row_ptr, A.col_ind, A.values,
        bundle.preconds,
        b,
        x,
...
void print_config(const AppOptions &opts,
                  const std::size_t subdomain_count)
{
    std::cout
        << "[Config] n=" << opts.n
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        GpuSubdomainBundle gpu_bundle = build_gpu_subdomain_bundle(A, opts);
        CpuSubdomainBundle cpu_bundle = build_cpu_subdomain_bundle(A, opts);
        print_config(opts, gpu_bundle.regions.size());

        SolverRun gpu_run = run_gpu_mpcg(A, b, opts, gpu_bundle); //Warmup
        gpu_run = run_gpu_mpcg(A, b, opts, gpu_bundle);
        print_run("[MPCG GPU]", gpu_run);

        const SolverRun cpu_run = run_cpu_mpcg(A, b, opts, cpu_bundle);
        print_run("[MPCG CPU]", cpu_run);

        const double gpu_end_to_end = gpu_run.precond_secs + gpu_run.solve_secs;
        const double cpu_end_to_end = cpu_run.precond_secs + cpu_run.solve_secs;
        if (gpu_end_to_end > 0.0)
            std::cout << "[Speedup CPU/GPU end_to_end] " << (cpu_end_to_end / gpu_end_to_end) << "x\n";
        if (gpu_run.solve_secs > 0.0)
            std::cout << "[Speedup CPU/GPU solve_only] " << (cpu_run.solve_secs / gpu_run.solve_secs) << "x\n";

        if (gpu_run.result.finalRes > opts.params.tol * stopping_scale)
            std::cerr << "[Warn] GPU MPCG did not reach the requested tolerance.\n";
        if (cpu_run.result.finalRes > opts.params.tol * stopping_scale)
            std::cerr << "[Warn] CPU MPCG did not reach the requested tolerance.\n";
    }
    catch (const std::exception &e)
    {
        std::cerr << "Error: " << e.what() << "\n";
