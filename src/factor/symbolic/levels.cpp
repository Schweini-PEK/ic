...
namespace ichol::symbolic
{
                                          ...
                                          const std::vector<std::pair<int, int>> &snodes)
    {
        ...
        int ncols = 0;
        if (num_col_levels >= 0)
            ncols = col_level_sets.level_ptr.back();
        ...
        for (int id = 0; id < m; ++id)
            snode_sets.level_ptr[snode_level[id] + 1]++;
    ...
    ichol::symbolic::LevelSets build_level_sets_upper_csr(
        const ichol::symbolic::FactorPattern &U_pattern)
    {
        int n = (int)U_pattern.row_ptr_L.size() - 1;
        int max_level = -1;
        std::vector<int> level_of(n, -1);

        // Reverse order: i = n-1 .. 0 for upper/backward dependencies
        for (int ii = 0; ii < n; ++ii)
        {
            int i = n - 1 - ii;

            int best = -1;
            // skip last entry (diagonal)
            for (int p = U_pattern.row_ptr_L[i]; p < U_pattern.row_ptr_L[i + 1] - 1; ++p)
            {
                int j = U_pattern.col_ind_L[p];
                best = std::max(best, level_of[j]);
            }
            level_of[i] = best + 1;
            max_level = std::max(max_level, level_of[i]);
        }
        ...
        level_sets.levels.resize(n);
        std::vector<int> next(level_sets.level_ptr.begin(), level_sets.level_ptr.end() - 1);
        for (int i = 0; i < n; ++i)
        {
            int L = level_of[i];
            level_sets.levels[next[L]++] = i;
