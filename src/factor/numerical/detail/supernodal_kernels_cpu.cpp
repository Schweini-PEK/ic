    ...
    SupernodalWorkspace& ws)
{
    ...
    // Add children updates (scatter-add), using g2l mapping
    for (int c : children[(size_t)k]) {
        const UpdatePack& uc = up[(size_t)c];
        const int m = uc.nupd;
        if (m <= 0) continue;

        ws.tmp_pos.resize((size_t)m);
        for (int a = 0; a < m; ++a) {
            ws.tmp_pos[(size_t)a] = ws.g2l[(size_t)uc.idx[(size_t)a]];
        }
        ...
        for (int j = 0; j < m; ++j) {
            const int pj = ws.tmp_pos[(size_t)j];
            if (pj < 0) continue;
            for (int i = j; i < m; ++i) {
                const int pi = ws.tmp_pos[(size_t)i];
                if (pi < 0) continue;
